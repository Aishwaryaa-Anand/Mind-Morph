...
export default function TwitterResult() {
  ...
  const loadResult = async () => {
    try {
      const data = await twitterService.getResultById(id);
      setResult(data);
      setLoading(false);
    } catch {
          ...
          <div className="glass-card mb-8">
            <div className="flex items-center gap-4">
              <div className="flex-1">
                ...
                <p className="text-white/70 mb-2">@{prediction.username}</p>
                {profileInfo.bio && (
        ...
        {keywords.length > 0 && (
          <div className="glass-card mb-8">
            ...
            <p className="text-white/80 mb-4">
              Words and phrases from tweets that influenced the prediction:
            </p>
            <div className="flex flex-wrap gap-2">
              {keywords.map((keyword, idx) => (
                <span
                  key={idx}
                  className="px-4 py-2 rounded-full bg-blue-500/30 border border-blue-500/50 text-white font-medium"
                >
                  {keyword}
                </span>
              ))}
